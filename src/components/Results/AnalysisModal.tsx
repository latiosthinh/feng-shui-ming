...
const ANALYSIS_LABELS: Record<AnalysisType, { label: string; emoji: string }> = {
...
export function AnalysisModal({ name, surname, birthDate, birthTime }: AnalysisModalProps) {
  ...
  const runAnalysis = async (type: AnalysisType) => {
    ...
    setLoading(type)
    try {
      const result = await analyzeNameAction(name, surname, type, birthDate, birthTime)
      ...
      setResults((prev) => ({ ...prev, [type]: "分析失败，请重试" }))
    } finally {
      setLoading(null)
    }
  }
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex gap-1 px-6 py-3 border-b border-gray-100 overflow-x-auto">
          {ANALYSIS_TYPES.map((type) => {
            const info = ANALYSIS_LABELS[type]
