...
export default function AdminClient({ adminToken }: AdminClientProps) {
  ...
  useEffect(() => {
    const pb = createPocketBase(adminToken)
    loadUsers(pb)
  }, [adminToken])

  const loadUsers = useCallback(async (pb: ReturnType<typeof createPocketBase>) => {
    try {
  ...
  const handleLogin = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Reload page to get server-side auth verification
